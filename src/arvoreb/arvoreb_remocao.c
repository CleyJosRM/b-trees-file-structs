...
/*
 * Coloca o nó no topo da pilha de nós removidos para
 *      reutilização futura. Decrementa contador de nós ativos.
 *      O conteúdo anterior do nó é preservado.
 * Recebe um ponteiro FILE* do arquivo de índice, o RRN do nó a marcar como livre
 *      e o cabeçalho da árvore-B para atualizar o topo da pilha de removidos e o contador de nós ativos.
 */
static void empilhar_pagina_livre(FILE* arvoreB, byteBTree* cabecalho, int rrnPagina){
